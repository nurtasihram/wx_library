...
struct WindowPosition : public RefStruct<WINDOWPOS> {
public:
	using Flag = WindowPosFlag;
public:
	WindowPosition() {}
	WindowPosition(const WINDOWPOS &w) : RefStruct<WINDOWPOS>(w) {}
...
class WindowClassBaseX :
	public RefStruct<std::conditional_t<IsUnicode, WNDCLASSW, WNDCLASSA>>,
...
public:
	...
	using Style = CStyle;
public:
	WindowClassBaseX() reflect_to(self->cbClsExtra = (int)sizeof(Child) - (int)sizeof(WNDCLASSEX));
public: // Property - Styles
...
public: // Property - Icon
	/* W */ inline auto &Icon(HICON hIcon) reflect_to_child(self->hIcon = hIcon);
...
public: // Property - Background
	...
	/* W */ inline auto &Background(SysColor brBackground) reflect_to_child(self->hbrBackground = (HBRUSH)brBackground.yield());
...
public: // Property - Name
	...
	/* R */ inline String Name() const {
		...
		if (IS_INTRESOURCE(self->lpszClassName))
			return sprintf("#%d", (ATOM)(ULONG_PTR)self->lpszClassName);
...
public:
	inline ATOM Register() const {
		if constexpr (IsUnicode)
			assertl_reflect_as(auto atom = RegisterClassW(self), atom)
		else
			assertl_reflect_as(auto atom = RegisterClassA(self), atom)
	}
	inline void Unregister() const {
		if constexpr (IsUnicode)
			assertl_reflect_as(UnregisterClassW(self->lpszClassName, self->hInstance))
		else
			assertl_reflect_as(UnregisterClassA(self->lpszClassName, self->hInstance))
	}
	inline auto &GetInfo() {
		if constexpr (IsUnicode)
			assertl_reflect_as_self(GetClassInfoW(self->hInstance, self->lpszClassName, self))
		else
			assertl_reflect_as_self(GetClassInfoA(self->hInstance, self->lpszClassName, self))
	}
};
...
template<bool IsUnicode, class AnyChild = void>
class WindowClassExBaseX :
	public RefStruct<std::conditional_t<IsUnicode, WNDCLASSEXW, WNDCLASSEXA>>,
...
public:
	inline ATOM Register() const {
		if constexpr (IsUnicode)
			assertl_reflect_as(auto atom = RegisterClassExW(self), atom)
		else
			assertl_reflect_as(auto atom = RegisterClassExA(self), atom)
	}
	inline auto &Unregister() const {
		if constexpr (IsUnicode)
			assertl_reflect_as_self(UnregisterClassW(self->lpszClassName, self->hInstance))
		else
			assertl_reflect_as_self(UnregisterClassA(self->lpszClassName, self->hInstance))
	}
	inline auto &GetInfo() {
		if constexpr (IsUnicode)
			assertl_reflect_as_self(GetClassInfoExW(self->hInstance, self->lpszClassName, self))
		else
			assertl_reflect_as_self(GetClassInfoExA(self->hInstance, self->lpszClassName, self))
	}
};
...
class CreateStructBaseX :
	public RefStruct<std::conditional_t<IsUnicode, CREATESTRUCTW, CREATESTRUCTA>>,
	public ChainExtend<CreateStructBaseX<IsUnicode, AnyChild, Style, StyleEx>, AnyChild> {
	using LPCTSTR = LPCXSTR<IsUnicode>;
	using String = StringX<IsUnicode>;
public:
	CreateStructBaseX() {}
	CreateStructBaseX(const CREATESTRUCT &cs) : RefStruct<CREATESTRUCT>(cs) {}
public: // Property - Param
...
public: // Property - Class
	...
	/* R */ inline String ClassName() const {
		...
		if (IS_INTRESOURCE(self->lpszClass))
			return sprintf("#%d", (ATOM)(ULONG_PTR)self->lpszClass);
...
public:
	...
	inline auto &Move(LRect rc, bool bRedraw = true) const assertl_reflect_as_child(::MoveWindow(self, rc.top, rc.left, rc.xsize(), rc.ysize(), bRedraw));
...
public:
	...
	static inline void Enum(AnyFunc fnEnumWindows) {
		if constexpr (static_compatible<AnyFunc, bool(HWND)>) {
			FnEnumWindow fnEnumWindows{
				[&](HWND hWnd) {
					return fnEnumWindows(hWnd);
				}
			};
	...
	template<class AnyFunc>
	inline void EnumChild(AnyFunc fnEnumChildren) {
		if constexpr (static_compatible<AnyFunc, bool(HWND)>) {
			FnEnumWindow fnEnumChildren{
				[&](HWND hWnd) {
					return fnEnumChildren(hWnd);
				}
			};
	...
	template<class AnyFunc>
	inline int EnumProp(AnyFunc fnEnumProps) {
		if constexpr (static_compatible<AnyFunc, bool(HWND, LPSTR, HANDLE)>) {
			FnEnumPropA fnEnumProps{
				[&](HWND hWnd, LPSTR lpString, HANDLE hData) {
					return fnEnumProps(hWnd, lpString, hData);
				}
			};
			return EnumPropsExA(self, EnumPropProc, (LPARAM)std::addressof(fnEnumProps));
		}
		else if constexpr (static_compatible<AnyFunc, bool(HWND, LPWSTR, HANDLE)>) {
		...
		else if constexpr (static_compatible<AnyFunc, void(HWND, LPSTR, HANDLE)>) {
			FnEnumPropA fnEnumProps{
				[&](HWND hWnd, LPSTR lpString, HANDLE hData) {
					fnEnumProps(hWnd, lpString, hData);
					return true;
				}
			};
			return EnumPropsExA(self, EnumPropProc, (LPARAM)std::addressof(fnEnumProps));
...
protected:
	...
	static const StringA &&_ClassNameA;
	static const StringW &&_ClassNameW;
public:
	...
	static inline auto &CClassNameA() reflect_as(_ClassNameA);
	static inline auto &CClassNameW() reflect_as(_ClassNameW);
	static inline ATOM CClassAtom() reflect_as(_ClassAtom);
protected:
...
public:
	inline ATOM Register() {
		...
		subtype_branchof_xClass<Child, XClassEx<>> cs;
		if constexpr (std::is_same_v<decltype(cs), XClassEx<>>)
			cs.Cursor(IDC_ARROW)
			  .Styles(ClassStyle::Redraw)
...
protected:
	...
	class XCreateBase :
		public WX::CreateStructBaseX<IsUnicode, KChain<XCreateBase<IsUnicode, SubCreate, Style, StyleEx>, SubCreate>, Style, StyleEx>,
		public ChainExtend<XCreateBase<IsUnicode, SubCreate, Style, StyleEx>, SubCreate> {
		WindowBase &_this;
	public:
		...
		using String = StringX<IsUnicode>;
	public:
		XCreateBase(WindowBase &_this) : _this(_this) {
...
public:
	inline auto Create() {
		Register();
		subtype_branchof_xCreate<Child, XCreate<>> cs(self);
		if constexpr (std::is_same_v<decltype(cs), XCreate<>>)
			cs.Size(CW_USEDEFAULT)
...
public: // Property - ModuleFileName
	/* R */ inline String ModuleFileName() const {
		auto lpszName = String::Alloc(MaxLenPath);
		int len = 0;
		assertl((len = GetWindowModuleFileName(self, lpszName, MaxLenPath)) > 0);
...
public: // Property - Prop
	...
	/* R */ inline HANDLE Prop(ATOM globalAtom) const assertl_reflect_to(auto hData = GetProp(self, MAKEINTATOM(globalAtom)), hData);
...
public:
	inline AnyChild *operator*() reflect_as(Long<AnyChild *>());
	inline AnyChild *operator->() reflect_as(Long<AnyChild *>());
	inline const AnyChild *operator*() const reflect_as(Long<const AnyChild *>());
	inline const AnyChild *operator->() const reflect_as(Long<const AnyChild *>());
public:
...
template<class AnyChild> const StringA &&WindowBase<AnyChild>::_ClassNameA = CString(typeid(typename WindowBase<AnyChild>::Child).name(), MaxLenClass);
template<class AnyChild> const StringW &&WindowBase<AnyChild>::_ClassNameW = FitsW(CString(typeid(typename WindowBase<AnyChild>::Child).name(), MaxLenClass));
