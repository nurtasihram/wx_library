...
public: // Property - FullScreen
	/* W */ inline auto&FullScreen(bool bFullScreen) reflect_to_self(WX::SetConsoleDisplayMode(hOutput, bFullScreen ? CONSOLE_FULLSCREEN_MODE : CONSOLE_WINDOWED_MODE, O));
//	/* A */ inline auto&FullScreenHardware() reflect_to_self(WX::SetConsoleDisplayMode(hOutput, CONSOLE_FULLSCREEN_HARDWARE, O));
	/* R */ inline bool FullScreen() const reflect_to(DWORD mode = 0; WX::GetConsoleDisplayMode(&mode), mode == CONSOLE_FULLSCREEN_MODE);
public: // Property - Mode
	/* W */ inline auto&Modes(Mode modes) reflect_to_self(WX::SetConsoleMode(hOutput, modes.yield()));
...
public: // Property - ScreenBufferInfo
	/* W */ inline auto&ScreenBufferInfo(CONSOLE_SCREEN_BUFFER_INFOEX sbiex) reflect_to_self(WX::SetConsoleScreenBufferInfo(hOutput, &sbiex));
...
public: // Property - ScreenBufferSize
	/* W */ inline auto &ScreenBufferSize(LSize size) reflect_to_self(WX::SetConsoleScreenBufferSize(hOutput, size));
	/* R */ inline LSize ScreenBufferSize() const reflect_as(ScreenBufferInfo().Size());
public: // Property - WindowSize
//	/* W */ inline auto&WindowSize(LRect rect) reflect_to_self(SetConsoleWindowInfo(hOutput, TRUE, ));
	/* R */ inline auto WindowSize() const reflect_as(ScreenBufferInfo().WindowRect().size());
public: // Property - Attributes
	/* W */ inline auto&Attributes(Attribute wAttributes) reflect_to_self(WX::SetConsoleTextAttribute(hOutput, wAttributes.yield()));
	/* R */ inline auto Attributes() const reflect_as(ScreenBufferInfo().Attributes());
public: // Property - CursorInfo
	/* W */ inline auto&CursorInfo(CONSOLE_CURSOR_INFO cci) reflect_to_self(WX::SetConsoleCursorInfo(hOutput, &cci));
	/* R */ inline auto CursorInfo() const reflect_to(ConsoleCursorInfo cci, WX::GetConsoleCursorInfo(hOutput, &cci), cci);
public: // Property - CursorPosition
	/* W */ inline auto&CursorPosition(LPoint p) reflect_to_self(WX::SetConsoleCursorPosition(hOutput, COORD({ (short)p.x, (short)p.y })));
